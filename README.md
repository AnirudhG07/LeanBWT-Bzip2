...
The large harness is intentionally gated for GB-scale workloads, but the exact
runtime now uses the separate fast forward BWT path. The current large matrix
includes a 1 GiB shell baseline on sparse data, a default 16 MiB exact sparse
interop case, and additional multi-megabyte / incompressible / many-block shell
coverage. The remaining gap is not the old reference BWT anymore; it is pushing
the exact runtime further toward fully streaming large-file behavior.
